...
class QueueRule:
    ...
    def get_pull_request_rule(
        ...
        return QueuesRulesEvaluator(
            [self], pull_request, EvaluatedQueueRule.from_rule, False
        ).matching_rules[0]
...
class GenericRulesEvaluator(typing.Generic[T_Rule, T_EvaluatedRule]):
    ...
    # evaluated rule getter
    evaluated_rule_getter: typing.Callable[
        ...
        for rule in self.rules:
            ...
            if ignore_rules and self.hide_rule:
                self.ignored_rules.append(
                    self.evaluated_rule_getter(rule, next_conditions_to_validate)
                )
            else:
...
class PullRequestRules:
    ...
    def get_pull_request_rule(self, ctxt: context.Context) -> RulesEvaluator:
        return RulesEvaluator(self.rules, ctxt, EvaluatedRule.from_rule, True)
