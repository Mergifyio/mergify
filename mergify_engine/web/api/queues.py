...
class SpeculativeCheckPullRequest:
    in_place: bool = dataclasses.field(
        metadata={"description": "Does the pull request has been checked in-place"}
    )
...
async def repository_queues(
    ...
    client: github.AsyncGithubInstallationClient = fastapi.Depends(  # noqa: B008
        security.get_github_client
    ),
) -> Queues:
    try:
    ...
    installation = context.Installation(
        repo["owner"]["id"], repo["owner"]["login"], sub, client, redis_cache
    )
