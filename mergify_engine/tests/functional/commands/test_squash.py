...
class TestCommandSquash(base.FunctionalTestBase):
    async def test_squash_several_commits_ok(self):
        ...
        pr = r.json()
        pr_number = pr["number"]
        commits = await self.get_commits(pr_number)
        assert len(commits) > 1

        # do the squash
        pull = await self.get_pull(pr_number)
        ctxt = await context.Context.create(self.repository_ctxt, pull, [])
        await squash_pull.squash_pull(ctxt)

        # get the PR
        pr = await client.item(f"{self.url_main}/pulls/{pr_number}")
        assert pr["commits"] == 1

    async def test_squash_several_commits_with_one_merge_commit(self):
        await self.setup_repo({})
        repo_name = "fork"
