...
class TestQueueAction(base.FunctionalTestBase):
    ...
    def test_basic_queue(self):
        ...
        self.add_label(p1, "queue")
        self.add_label(p2, "queue")
        self.run_engine()
        ...
        assert q.train._cars[0].pull_request_number == p1.number
        assert q.train._cars[1].pull_request_number == p2.number
    ...
    def test_ongoing_train_basic(self):
        rules = {
            "queue_rules": [
                {
                    "name": "default",
        ...
        assert q.train._cars[0].pull_request_number == p1.number
        tmp_p1 = first(
            pulls, key=lambda p: p.number == q.train._cars[0].tmp_pull_request_number
        )
        self.create_status(tmp_p1)
        self.run_engine()
        self.wait_for("push", {"ref": f"refs/heads/{self.master_branch_name}"})
        self.run_engine(3)
        pulls = list(self.r_o_admin.get_pulls())
        assert len(pulls) == 3
        q = queue.Queue.from_context(ctxt, with_train=True)
        ...
        assert q.train._cars[0].pull_request_number == p2.number
        ...
        assert q.train._cars[0].parents == [p1.number]

        # Ensure base is p1 and only p2 is tested with p3
        assert q.train._cars[1].pull_request_number == p3.number
        ...
        assert q.train._cars[1].parents == [p2.number]

    def test_ongoing_train_second_pr_ready_first(self):
        rules = {
            "queue_rules": [
                {
                    "name": "default",
        ...
        tmp_p2 = first(
            pulls, key=lambda p: p.number == q.train._cars[1].tmp_pull_request_number
        )
        self.create_status(tmp_p2)
        self.run_engine()
        pulls = list(self.r_o_admin.get_pulls())
        assert len(pulls) == 4
        q = queue.Queue.from_context(ctxt, with_train=True)
    ...
    def test_queue_ci_failure(self):
        rules = {
            "queue_rules": [
                {
                    "name": "default",
        ...
        self.create_status(tmp_p1, state="failure")
        self.run_engine()
    ...
    def test_queue_cant_create_tmp_pull_request(self):
        rules = {
            "queue_rules": [
                {
                    "name": "default",
        ...
        self.setup_repo(yaml.dump(rules))
        p1, _ = self.create_pr(files={"conflicts": "well"})
        p2, _ = self.create_pr(files={"conflicts": "boom"})
        p3, _ = self.create_pr()
        ...
        p2.update()
        ctxt_p2 = context.Context(self.cli_integration, p2.raw_data, {})
        check = first(
            ctxt_p2.pull_engine_check_runs,
            key=lambda c: c["name"] == constants.MERGE_QUEUE_SUMMARY_NAME,
        )
        assert (
            check["output"]["title"] == "This pull request cannot tested to be merged"
        )
        assert (
            check["output"]["summary"]
            == "This pull request can't be tested to be merged because: Merge conflict"
        )
    ...
    def test_queue_manual_merge(self):
        rules = {
            "queue_rules": [
                {
                    "name": "default",
        ...
        ctxt = context.Context(self.cli_integration, p3.raw_data, {})
        q = queue.Queue.from_context(ctxt, with_train=True)
        pulls_in_queue = q.get_pulls()
        assert pulls_in_queue == [p1.number, p2.number]
    ...
    def test_queue_priority(self):
        ...
        self.add_label(p2, "queue-urgent")
        self.run_engine()
...
class TestTrainApiCalls(base.FunctionalTestBase):
    ...
    def test_create_pull_basic(self):
        self.setup_repo(yaml.dump({}))
        ...
        car.create_pull()
        assert car.tmp_pull_request_number is not None
        pulls = list(self.r_o_admin.get_pulls())
        assert len(pulls) == 3

        old_tmp_pull_request_number = car.tmp_pull_request_number
        car.create_pull()
        pulls = list(self.r_o_admin.get_pulls())
        assert len(pulls) == 3

        assert car.tmp_pull_request_number != old_tmp_pull_request_number
        car.delete_pull()
        pulls = list(self.r_o_admin.get_pulls())
        assert len(pulls) == 2

    def test_create_pull_conflicts(self):
        self.setup_repo(yaml.dump({}), files={"conflicts": "foobar"})

        p, _ = self.create_pr(files={"conflicts": "well"})
        p1, _ = self.create_pr()
        p2, _ = self.create_pr()
        p3, _ = self.create_pr(files={"conflicts": "boom"})
        ...
        with pytest.raises(merge_train.TrainCarPullRequestCreationFailure) as exc_info:
            ...
            assert car.tmp_pull_request_number is None

        p3.update()
        ctxt = context.Context(self.cli_integration, p3.raw_data, {})
        check = first(
            ctxt.pull_engine_check_runs,
            key=lambda c: c["name"] == constants.MERGE_QUEUE_SUMMARY_NAME,
        )
        assert (
            check["output"]["title"] == "This pull request cannot tested to be merged"
